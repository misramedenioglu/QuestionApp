...
function App() {
  ...
  const [timeLeft, setTimeLeft] = useState(30); // Kalan süreyi tutar
  ...
  // Sonraki soruya geçiş işlevi
  const handleNextQuestion = useCallback(() => {
    setShowOptions(false); // Seçenekleri gizle
    setShowNextButton(false); // "Sonraki Soru" butonunu gizle
    if (currentQuestion < questions.length - 1) { // Eğer daha fazla soru varsa
    ...
    } else { // Tüm sorular tamamlandıysa
      setQuizEnded(true); // Quiz bitti
      setQuizStarted(false); // Quizin başlangıcını sıfırla
    }
  }, [currentQuestion]);
  ...
  }, [quizStarted, currentQuestion, handleNextQuestion]);
